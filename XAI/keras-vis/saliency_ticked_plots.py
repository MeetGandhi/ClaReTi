...
os.mkdir("saliency_ticked_plots_viridify_sqrt\\")

for cls in classes:
    
    os.mkdir("saliency_ticked_plots_viridify_sqrt\\"+cls+"\\")
    blank = glob.glob("saliency\\"+cls+"\\*.png")
    ...
    for r in random_blank:
        ...
        fig.colorbar(im2, cax=cax2, boundaries=np.linspace(0,1,10))
        plt.tight_layout()
        #plt.suptitle("Class: "+cls+",  Saliency Maps\n"+"X_id: "+str(random_id)+",  Pred_Prob: "+str(round(prob[random_id, pd[cls]],4)))
        #plt.savefig("saliency_ticked_plots_viridify\\"+cls+"\\"+str(random_id)+".png")
        plt.savefig("saliency_ticked_plots_viridify_sqrt\\"+cls+"\\"+str(random_id)+"_"+str(round(prob[random_id, pd[cls]],4))+".png")
        plt.close()
