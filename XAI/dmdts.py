...
##    x_mean = x_sum/length
##
##    np.save("mean_dmdt_plots/"+pd[cls]+".npy", x_mean[:,:,0])

for j in indexes:
    plt.figure()
    fig, ax=plt.subplots(1,1)
    im1 = ax.imshow(x_test[j][:,:,0])
    im2 = ax.imshow(np.sqrt(x_test[j][:,:,0]))
    ...
    plt.tight_layout()
    plt.savefig("dmdt_plots_sqrt/"+str(j)+".png")
    plt.cla()
    plt.clf()
    plt.close()
